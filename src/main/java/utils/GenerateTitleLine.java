...
public class GenerateTitleLine {
	...
	String[] molecularFPTitle = new String[FingerPrintQueries_new.moleQueryList.size()];
	String[] currentBondFPTitle = new String[FingerPrintQueries_new.queriesList.size()];
	String[] currentBondAtomTypeTitle = new String[GenerateNeighborFeatures_new.atomTypeLookupTable.length];
	...
	public GenerateTitleLine(int depth_neighborAtomType, int depth_neighborAtomDescriptor){
	...
	public void generateCurrentBondAtomTypeTitle(){//4th
		for(int i = 0; i < GenerateNeighborFeatures_new.atomTypeLookupTable.length; i++){
			currentBondAtomTypeTitle[i] = "CurrentBond_" + GenerateNeighborFeatures_new.atomTypeLookupTable[i];
		}
	}
	...
	public void generateNeighborAtomTypeTitle(){
		neighborAtomTypeTitle = new String[GenerateNeighborFeatures_new.atomTypeLookupTable.length*depth_neighborAtomType];
		for(int d = 1; d < depth_neighborAtomType + 1; d++){
			for(int i = 0; i < GenerateNeighborFeatures_new.atomTypeLookupTable.length; i++){
				int idx = i + (d-1)*GenerateNeighborFeatures_new.atomTypeLookupTable.length;
				neighborAtomTypeTitle[idx] = "Neighbor_d=" + d +"_" + GenerateNeighborFeatures_new.atomTypeLookupTable[i];
			}
		}
	}

	public void generateNeighborAtomDescriptorTitle(){
		neighborAtomDescriptorTitle = new String[GenerateNeighborFeatures_new.roughAtomTypeTable.length*depth_neighborAtomDescriptor*10];
		for(int d = 1; d < depth_neighborAtomDescriptor + 1; d++){
			for(int i = 0; i < GenerateNeighborFeatures_new.roughAtomTypeTable.length; i++){
				for(int j = 0; j < atomicDescriptorTitle.length; j++){
					int idx = j + i*atomicDescriptorTitle.length + (d-1)*GenerateNeighborFeatures_new.roughAtomTypeTable.length*atomicDescriptorTitle.length;
					//System.out.println("d=" + d + "," + "i=" + i + ",j=" + j +",idx="+ idx);
					neighborAtomDescriptorTitle[idx] = "Neighbor_d=" + d + "_" + GenerateNeighborFeatures_new.roughAtomTypeTable[i] + "_" + atomicDescriptorTitle[j];
