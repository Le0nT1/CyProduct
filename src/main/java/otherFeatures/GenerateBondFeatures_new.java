...
public class GenerateBondFeatures_new {	
	...
	public String generateCurrentBondAtomType(IAtomContainer molecule, IBond oneBond) throws Exception {
		...
		String[] atomTypeLookupTable = GenerateNeighborFeatures_new.atomTypeLookupTable;
		...
		try {
			IAtomType atomType = typeMatcher.findMatchingAtomType(molecule, rightAtom); 
			type = atomType.getAtomTypeName();	
		}catch (Exception e) {
	...
	public String generateBondFingeprint(IAtomContainer molecule, IBond bond) throws Exception {
		...
		String[] matched = new String[FingerPrintQueries_new.queriesList.size()];
		...
		Iterator<Entry<String, String>> allQueries = FingerPrintQueries_new.queriesList.entrySet().iterator();
...
//		}
	...
	public List<IAtom> reorderAtomsOfBond(IAtomContainer molecule, IBond oneBond) throws Exception{
		...
		/*
		if(leftAtom.getSymbol().contains("C")&&rightAtom.getSymbol().contains("C")){
			...
			//result[0] = (length_left + length_right) /2;
			//result[1] = (length_left + length_right) /2;
		}
