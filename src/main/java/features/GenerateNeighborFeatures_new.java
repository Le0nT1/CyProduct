...
public class GenerateNeighborFeatures_new {
	...
	public static HashMap<String, String[]> generateNeighborAtomDescriptorMap(IAtomContainer molecule, IBond oneBond, int depth) throws Exception{
		...
		for(int i = 0; i < neighborAtoms.size(); i++){
			...
			String roughType = getConvertType(type);
			
			String[] newValue = GenerateAtomFeatures_ForBond.generateAtomDescriptorFeatures(molecule, oneAtom);
